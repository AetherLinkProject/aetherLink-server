...
public class GraphQLHelper : IGraphQLHelper, ISingletonDependency
{
    ...
    private readonly GraphqlOption _options;
    private readonly ILogger<GraphQLHelper> _logger;

    public GraphQLHelper(IGraphQLClient client, ILogger<GraphQLHelper> logger, IOptionsSnapshot<GraphqlOption> options)
    {
        _client = client;
        _logger = logger;
    ...
    private async Task<T> SendQueryAsync<T>(GraphQLRequest request)
    {
        ...
        try
        {
            ...
            _logger.LogError("[GraphQL] Query graphQL err, errors = {Errors}",
                string.Join(",", graphQlResponse.Errors.Select(e => e.Message).ToList()));
            return default;
        }
        catch (OperationCanceledException)
        {
            _logger.LogError("[GraphQL] Query graphQL timed out, Took {time} ms.",
                DateTime.Now.Subtract(startTime).TotalMilliseconds);
            throw;
        }
        catch (Exception e)
        {
            _logger.LogError(e, "[GraphQL] Query graphQL fail.");
            throw;
        }
    }
}