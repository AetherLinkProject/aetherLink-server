...
public interface IStateProvider
{
    ...
    public Dictionary<int, byte[]> GetCrossChainMultiSignature(string id);
    public void SetCrossChainMultiSignature(string id, Dictionary<int, byte[]> signatures);
...
public class StateProvider : IStateProvider, ISingletonDependency
{
    ...
    private readonly ConcurrentDictionary<string, Dictionary<int, byte[]>> _crossChainMultiSignatures = new();
    ...
    public Dictionary<int, byte[]> GetCrossChainMultiSignature(string id)
        => _crossChainMultiSignatures.TryGetValue(id, out var sign) ? sign : null;

    public void SetCrossChainMultiSignature(string id, Dictionary<int, byte[]> signatures)
        => _crossChainMultiSignatures[id] = signatures;
}