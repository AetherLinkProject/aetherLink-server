...
public class EvmSearchWorker : AsyncPeriodicBackgroundWorkerBase
{
    ...
    private async Task SearchRequestsAsync(EvmOptions options)
    {
        ...
        try
        {
            ...
            if (consumedBlockHeight + options.SubscribeBlocksStep + options.SubscribeBlocksDelay >= latestBlock)
            {
                _logger.LogDebug(
            ...
            var targetBlockHeight = consumedBlockHeight + options.SubscribeBlocksStep;
            _logger.LogInformation($"[EvmSearchWorker] {networkName} Processed blocks from {from} to {latestBlock}.");
            for (var curFrom = from; curFrom <= targetBlockHeight; curFrom += EvmSubscribeConstants.SubscribeBlockStep)
            {
                var currentTo = Math.Min(curFrom + EvmSubscribeConstants.SubscribeBlockStep - 1, targetBlockHeight);
