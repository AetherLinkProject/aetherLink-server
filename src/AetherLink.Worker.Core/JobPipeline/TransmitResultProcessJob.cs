...
public class TransmitResultProcessJob : AsyncBackgroundJob<TransmitResultProcessJobArgs>, ITransientDependency
{
    ...
    [ExceptionHandler(typeof(Exception), TargetType = typeof(TransmitResultProcessJob),
        MethodName = nameof(HandleException))]
    public virtual async Task Handler(TransmitResultProcessJobArgs args)
    {
    ...
    public async Task<FlowBehavior> HandleException(Exception ex, TransmitResultProcessJobArgs args)
    {
        _logger.LogError(ex, "[Step6] Failed, reqId:{ReqId}, epoch:{Epoch}", args.RequestId, args.Epoch);
