...
public class AutomationStart : AsyncBackgroundJob<AutomationStartJobArgs>, ITransientDependency
{
    ...
    public AutomationStart(IPeerManager peerManager, ILogger<AutomationStart> logger, IObjectMapper mapper,
        IBackgroundJobManager jobManager, ISchedulerService schedulerService, IJobProvider jobProvider,
        IOracleContractProvider contractProvider, ISignatureProvider signatureProvider)
    {
        _logger = logger;
        _mapper = mapper;
    ...
    public override async Task ExecuteAsync(AutomationStartJobArgs args)
    {
        ...
        try
        {
            ...
            if (_peerManager.IsLeader(args.Epoch, args.RoundId))
            {
                ...
                _signatureProvider.LeaderInitMultiSign(request.Context, originInput.PerformData.ToByteArray());
        ...
        {
            _logger.LogError(e, "[step1] {name} Automation Start process failed.", argId);
        }
    }
}