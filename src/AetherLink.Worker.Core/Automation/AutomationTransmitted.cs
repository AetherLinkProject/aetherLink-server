...
public class AutomationTransmitted : AsyncBackgroundJob<BroadcastTransmitResultArgs>, ITransientDependency
{
    ...
    [ExceptionHandler(typeof(Exception), TargetType = typeof(AutomationTransmitted),
        MethodName = nameof(HandleException))]
    public virtual async Task Handle(BroadcastTransmitResultArgs args)
    {
    ...
    public async Task<FlowBehavior> HandleException(Exception ex, BroadcastTransmitResultArgs args)
    {
        _logger.LogError(ex,
            $"[Automation]Check transmit args failed, upkeep:{args.Context.RequestId}, epoch:{args.Context.Epoch}");
